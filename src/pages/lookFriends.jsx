    ...
    <>
      <section className="h-[80vh] flex overflow-hidden rounded-xl shadow-lg mx-6 mb-6 font-serif">
        <div className="w-3/12 bg-[#dce1f9] p-6 m-2 flex flex-col rounded-lg">
          <h2 className="text-lg font-semibold mb-4 text-[#280f91] border-b border-[#280f91]/30 pb-3 font-serif">
            CONVERSACIONES CON TUS AMIGOS
          </h2>
          <input
            type="text"
            className="w-full px-5 py-3 mb-6 rounded-full border-2 border-[#dce1f9] bg-[#dce1f9] text-[#280f91] placeholder-[#280f91]/60 font-serif focus:outline-none focus:ring-4 focus:ring-[#280f91]/40 transition"
          ...
          <div
            name="chatList"
            className="flex flex-col gap-4 overflow-auto scrollbar-thin scrollbar-thumb-[#280f91]/60 scrollbar-track-[#dce1f9]"
          >
            {filteredUsers.map((user) => (
              <div
                key={user._id}
                name="chatItem"
                className="flex items-center gap-4 cursor-pointer p-4 rounded-lg hover:bg-[#280f91]/10 transition"
                onClick={() => roomNavegation(id, user)}
              >
                <img
                  src={user.profilePicture || "https://via.placeholder.com/150"}
                  alt={user.name}
                  className="rounded-full w-14 h-14 object-cover flex-shrink-0 border-2 border-[#280f91]"
                />
                <div className="flex flex-col">
                  <span className="font-semibold text-[#280f91] font-serif">
                    {user.name}
                  </span>
                  <small
                    className={`text-sm ${
        ...
        <div className="w-9/12 bg-[#280f91]/10 text-[#280f91] p-8 m-2 flex flex-col rounded-lg overflow-hidden font-serif">
          {showChat ? (
            <Chat
            ...
            <div className="flex items-center justify-center h-full text-[#280f91]/70 text-lg italic font-serif">
